...
- `box/` contains the vector files to cut the necessary holes in the MJLO box. It also contains the 3D-files for a)the carrier to insert into the box and carry the PCB and b) a backplane to secure a velcro strap to the box, and the vector outline file that serves as basis for this insert and the PCB shape.
...
### Printing the carrier and backplane
...
You can add a backplane to the box. The backplane makes it easy to secure a velcro strap to the box for mounting in various places. This is optional but recommended.
